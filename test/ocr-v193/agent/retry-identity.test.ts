...
describe("ocr-v193 agent retry identity (ported from internal/agent/retry_identity_test.go)", () => {
  ...
  test("TestExecutePlanPhase_Identity", async () => {
    for (const provider of ["openai", ""]) {
      ...
      const recs = sess.GetOrCreateFileSession("main.go").taskRecords.get("plan_task" as unknown as import("../../../src/ocr-v193/session/history.js").TaskType);
  ...
  test("TestExecuteReviewFilter_Identity", async () => {
    ...
    expect(meta.requestNo).toBe(1);
    const recs = sess.GetOrCreateFileSession("a.go").taskRecords.get("review_filter_task" as unknown as import("../../../src/ocr-v193/session/history.js").TaskType);
    expect(recs?.length).toBe(1);
    expect(meta.requestNo).toBe(recs![0]!.requestNo);
  });
  ...
  test("TestNewRequestMeta_IsSingleSourceOfProviderAndModel", () => {
    ...
    const got = (agent as unknown as { newRequestMeta: (f: string, t: import("../../../src/ocr-v193/session/history.js").TaskType, n: number) => CapturedMeta }).newRequestMeta("dir/f.go", "main_task" as unknown as import("../../../src/ocr-v193/session/history.js").TaskType, 3);
