...
import { ScriptedTransport } from "../../../src/ocr-v193/llmloop/transcript.js";
...
describe("ocr-v193 Agent review filter (ported)", () => {
  test("no filter task → no LLM call, comments unchanged", async () => {
    const collector = makeCollectorWith(cm("a.go", "keep"));
    let calls = 0;
    const llmClient: any = {
      complete: async () => { calls++; return { content: `["c-0"]` }; },
      CompletionsWithCtx: async (_sig: AbortSignal, req: unknown) => { calls++; return { content: `["c-0"]` }; },
    };
    ...
    await (agent as unknown as { executeReviewFilter: (sig: AbortSignal, d: Diff, p: string) => Promise<void> }).executeReviewFilter(new AbortController().signal, { newPath: "a.go", diff: "+code" } as unknown as Diff, "a.go");
    expect(calls).toBe(0);
  ...
  test("no comments → no LLM call", async () => {
    ...
    let calls = 0;
    const llmClient: any = {
      complete: async () => { calls++; return { content: `[]` }; },
    };
    ...
    await (agent as unknown as { executeReviewFilter: (sig: AbortSignal, d: Diff, p: string) => Promise<void> }).executeReviewFilter(new AbortController().signal, { newPath: "a.go", diff: "+x" } as unknown as Diff, "a.go");
    expect(calls).toBe(0);
  });
  ...
  test("removes comments via filter ids", async () => {
    ...
    const llmClient: any = {
      complete: async (_sig: AbortSignal, _req: unknown) => ({ content: `["c-1"]`, usage: { PromptTokens: 10, CompletionTokens: 5 } }),
      CompletionsWithCtx: async (_sig: AbortSignal, _req: unknown) => ({ content: `["c-1"]`, usage: { PromptTokens: 10, CompletionTokens: 5 } }),
    };
    const template = makeTemplate({
  ...
  test("out-of-range and invalid ids ignored", async () => {
    ...
    await (agent as unknown as { executeReviewFilter: (sig: AbortSignal, d: Diff, p: string) => Promise<void> }).executeReviewFilter(new AbortController().signal, { newPath: "a.go", diff: "+x" } as unknown as Diff, "a.go");
    // c-99 ignored, bad ignored, c-0 and c-1 valid → both removed
  ...
  test("filter respects abort signal (timeout) → keeps comments", async () => {
    const collector = makeCollectorWith(cm("a.go", "comment"));
    const llmClient: any = {
      complete: async (_sig: AbortSignal, _req: unknown) => {
        // Simulate abort by checking signal
        // In real test we abort before call; the agent's signal will be already aborted, and client should throw
    ...
    await (agent as unknown as { executeReviewFilter: (sig: AbortSignal, d: Diff, p: string) => Promise<void> }).executeReviewFilter(controller.signal, { newPath: "a.go", diff: "+x" } as unknown as Diff, "a.go");
    expect(collector.Comments().length).toBe(1);
  });

  test("concurrent comment processing with per-file draining then filter", async () => {
    // Simulate full subtask: loop produces comments via pool, then filter removes one
    const collector = new CommentCollector();
    collector.Add(cm("a.go", "keep"));
    collector.Add(cm("a.go", "remove"));
    collector.Add(cm("a.go", "keep2"));
    // Filter will remove c-1
    const llmClient: any = {
      complete: async (_sig: AbortSignal, req: unknown) => {
        const r = req as { messages: readonly { content: string }[] };
        // Distinguish plan vs filter vs main: main not used here, so just return filter response
        // Check if messages contain Filter pattern
      ...
      CompletionsWithCtx: async (_sig: AbortSignal, req: unknown) => {
        const r = req as { messages: readonly { content: string }[] };
        const content = JSON.stringify(r.messages);
        if (content.includes("Filter")) {
          return { content: `["c-1"]` };
    ...
    // Simulate that pool had async work for a.go that already completed (collector already has 3)
    await (agent as unknown as { executeReviewFilter: (sig: AbortSignal, d: Diff, p: string) => Promise<void> }).executeReviewFilter(new AbortController().signal, { newPath: "a.go", diff: "+code" } as unknown as Diff, "a.go");
    const remaining = collector.CommentsForPath("a.go");
    expect(remaining.length).toBe(2);
