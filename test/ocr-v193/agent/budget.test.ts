...
import type { ChatRequest, ChatResponse } from "../../../src/ocr-v193/llmloop/types.js";
import { StatePartial, StateFailed, StateComplete, FailureBudget } from "../../../src/ocr-v193/session/manifest.js";
...
describe("ocr-v193 Agent budget (ported from internal/agent/budget_test.go)", () => {
  ...
  test("dispatch stops via token look-ahead with warning and partial manifest", async () => {
    ...
    expect(err).toBeNull();
    const manifest = (agent as unknown as { RunManifest: () => import("../../../src/ocr-v193/session/manifest.js").RunManifest | null }).RunManifest();
