...
import { runReviewContext, type ReviewRunner } from "../../../src/ocr-v193/cli/review.js";
import type { ReviewOptions } from "../../../src/ocr-v193/cli/shared.js";
import { defaultReviewOptions } from "../../../src/ocr-v193/cli/shared.js";
  ...
  test("retry report appears via packed pi-review binary (429 then success)", async () => {
    ...
    const runPi = async (format: string): Promise<{ stdout: string; stderr: string; exitCode: number | null }> => {
      ...
      const args = format === "json"
        ? ["review", "--repo", repoDir, "--model", "test-openai/test-model", "--concurrency", "1", "--json"]
        : ["review", "--repo", repoDir, "--model", "test-openai/test-model", "--concurrency", "1"];
