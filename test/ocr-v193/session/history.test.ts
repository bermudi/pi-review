...
import { newTextMessage } from "../../../src/ocr-v193/llmloop/compression.ts";

describe("ocr-v193 session history", () => {
  // OCR v1.9.3: TestNew
  ...
  test("AppendTaskRecord increments per-task counters", () => {
    ...
    const msgs = [newTextMessage("user", "hello")] as unknown as import("../../../src/ocr-v193/session/history.ts").Message[];
  ...
  test("AppendTaskRecord stores a defensive copy of messages", () => {
    const sh = new SessionHistory("/tmp/repo", "main", "model", {});
    const fs = sh.GetOrCreateFileSession("file.go");
    const msgs = [newTextMessage("user", "original")] as unknown as import("../../../src/ocr-v193/session/history.ts").Message[];
    const rec = fs.AppendTaskRecord("main_task", msgs);
  ...
  test("SetResponse records content, model, usage and duration", () => {
    const sh = new SessionHistory("/tmp/repo", "main", "model", {});
    const fs = sh.GetOrCreateFileSession("file.go");
    const rec = fs.AppendTaskRecord("main_task", [newTextMessage("user", "hi")] as unknown as import("../../../src/ocr-v193/session/history.ts").Message[]);
    const resp = {
