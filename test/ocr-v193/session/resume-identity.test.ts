...
import { SessionFilePath, JsonlWriter } from "../../../src/ocr-v193/session/persist.ts";
import {
...
import type { RunIdentity, ResumeRequest, ResumeLineage } from "../../../src/ocr-v193/session/resume.ts";
...
describe("ocr-v193 session resume identity", () => {
  ...
  test("resume lineage round-trips through session file", () => {
    ...
    try {
      ...
      const sh = new SessionHistory(repoDir, "feature", "gpt-5", { reviewMode: "range", diffFrom: "main", diffTo: "feature", resumedFrom: "parent-session", operation: "review" } as unknown as import("../../../src/ocr-v193/session/history.ts").SessionOptions);
      attachTestWriter(sh);
  ...
  test("resume lineage reaches disk before finalize", () => {
    ...
    try {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ocr-repo-"));
      const sh = new SessionHistory(repoDir, "feature", "gpt-5", { reviewMode: "range", operation: "review" } as unknown as import("../../../src/ocr-v193/session/history.ts").SessionOptions);
      attachTestWriter(sh);
  ...
  test("unknown lineage schema is ignored", () => {
    ...
    try {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ocr-repo-"));
      const sh = new SessionHistory(repoDir, "main", "model", { reviewMode: "range", operation: "review" } as unknown as import("../../../src/ocr-v193/session/history.ts").SessionOptions);
      attachTestWriter(sh);
      const fp = SessionFilePath(repoDir, sh.sessionId);
      sh.Finalize();
