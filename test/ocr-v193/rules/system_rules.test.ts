...
describe("SystemRulesIntegrity", () => {
  ...
  test("no_orphan_files", () => {
    const raw = readFileSync(join(process.cwd(), "src/ocr-v193/rules/system_rules.json"), "utf-8");
    ...
    const entries = readdirSync(join(process.cwd(), "src/ocr-v193/rules/rule_docs"));
    for (const e of entries) {
      if (e === "PROVENANCE.json") continue;
      // Skip directories; only files
      try {
        const st = statSync(join(process.cwd(), "src/ocr-v193/rules/rule_docs", e));
