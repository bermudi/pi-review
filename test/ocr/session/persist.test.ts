...
import { encodeRepoPath, SessionFilePath, JsonlWriter, SessionsDir } from "../../../src/ocr/session/persist.ts";
...
describe("ocr session persist", () => {
  ...
  test("SetError writes llm_error JSONL record", () => {
    ...
    try {
      ...
      const sh = new SessionHistory(repoDir, "main", "test-model", { reviewMode: "workspace" } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      attachTestWriter(sh);
  ...
  test("Finalize surfaces writer creation error without stdout", () => {
    ...
    const origHome = process.env.HOME;
      ...
      const sh2 = new SessionHistory(sh.repoDir, "main", "test-model", { reviewMode: "workspace" } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
  ...
  test("Finalize surfaces write error when file is closed", () => {
    ...
    const origHome = process.env.HOME;
        ...
        try {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ocr-repo-"));
      const sh2 = new SessionHistory(repoDir, "main", "test-model", { reviewMode: "workspace" } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      const handle = {
        writeReviewItemDone: () => {},
  ...
  test("review item done persists and is reusable via resume", () => {
    ...
    try {
      ...
      const sh = new SessionHistory(repoDir, "main", "test-model", { reviewMode: "workspace" } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      attachTestWriter(sh);
      sh.RecordReviewItemDone("a.go", "a.go", "a.go", "fp-a", [{ path: "a.go", content: "fix" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment]);
      sh.Finalize();
      const state = ResumeState.prototype as unknown as { constructor: unknown };
      // Use LoadResumeState to verify item persisted
      const { LoadResumeState } = require("../../../src/ocr/session/resume.ts") as typeof import("../../../src/ocr/session/resume.ts");
  ...
  test("ResumeState keeps repoDir when cwd empty in session_start", () => {
    ...
    try {
      ...
      const { ResumeState: RS } = require("../../../src/ocr/session/resume.ts") as typeof import("../../../src/ocr/session/resume.ts");
  ...
  test("scan path scope resume round-trip", () => {
    ...
    try {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ocr-repo-"));
      const sh = new SessionHistory(repoDir, "main", "test-model", { reviewMode: "full_scan", scanPaths: ["src", "lib"] } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      attachTestWriter(sh);
      sh.Finalize();
      const { LoadResumeState } = require("../../../src/ocr/session/resume.ts") as typeof import("../../../src/ocr/session/resume.ts");
      const loaded = LoadResumeState(repoDir, sh.sessionId);
  ...
  test("whole repo scan path scope resume round-trip", () => {
    ...
    try {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ocr-repo-"));
      const sh = new SessionHistory(repoDir, "main", "test-model", { reviewMode: "full_scan", scanPaths: [] } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      attachTestWriter(sh);
      sh.Finalize();
      const { LoadResumeState } = require("../../../src/ocr/session/resume.ts") as typeof import("../../../src/ocr/session/resume.ts");
      const loaded = LoadResumeState(repoDir, sh.sessionId);
