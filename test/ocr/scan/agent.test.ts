...
// localPath test/ocr/scan/agent.test.ts -> internal/scan/agent_test.go
...
import { Agent, NewAgent, formatPlanGuidance, buildSummaryCommentsList, scanItemFingerprint } from "../../../src/ocr/scan/scan.js";
...
describe("ocr scan agent (ported from internal/scan/agent_test.go)", () => {
  ...
  test("TestInjectScanContentMap", async () => {
    ...
    const { Registry } = await import("../../../src/ocr/tool/definitions.js");
    const { FileReadProvider } = await import("../../../src/ocr/tool/filereader.js");
    const { FileReader } = await import("../../../src/ocr/tool/filereader.js");
    const { ModeWorkspace } = await import("../../../src/ocr/diff/git.js");
