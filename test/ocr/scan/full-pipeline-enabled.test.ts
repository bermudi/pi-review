...
describe("ocr scan full pipeline enabled", () => {
  test("Run with Plan/Dedup/Summary enabled produces summary and deduped comments", async () => {
    ...
    try {
      ...
      const comments = await (agent as unknown as { run: (s?: AbortSignal) => Promise<import("../../../src/ocr/model/review.js").LlmComment[]> }).run(new AbortController().signal);
