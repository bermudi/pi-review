...
import type { ChatRequest, ChatResponse, RequestMeta } from "../../../src/ocr/llmloop/types.ts";
import { CompressionState, newTextMessage, type Message, type ToolCall } from "../../../src/ocr/llmloop/compression.ts";
import { SessionHistory } from "../../../src/ocr/session/history.ts";
import type { TaskType } from "../../../src/ocr/session/history.ts";
...
function runnerDeps(
  ...
  overrides: Partial<import("../../../src/ocr/llmloop/types.ts").RunnerDeps> = {},
): import("../../../src/ocr/llmloop/types.ts").RunnerDeps {
  return {
    model: "fake",
    template: {
      MaxTokens: 100_000,
    ...
    llmClient: transport as unknown as import("../../../src/ocr/llmloop/types.ts").AnyLlmClient,
    mainToolDefs: [
    ...
    ...overrides,
  } as unknown as import("../../../src/ocr/llmloop/types.ts").RunnerDeps;
}

describe("ocr llmloop identity", () => {
  ...
  test("main task rounds carry per-round RequestNo identity", async () => {
    for (const provider of ["openai", ""]) {
      ...
      } as unknown as Partial<import("../../../src/ocr/llmloop/types.ts").RunnerDeps>);
  ...
  test("relocation request identity follows comment path", async () => {
    ...
    const runner = new Runner(
      runnerDeps(transport, {
        session: sess,
        newRequestMeta: metaFactory("openai", "fake"),
        commentCollector: collector as unknown as import("../../../src/ocr/llmloop/types.ts").CommentCollectorLike,
        toolRegistry: new Map(),
        diffLookup: (path: string) => ({ newPath: path, newFileContent: "line one\nline two\n", diff: "@@ -1 +1 @@" }) as unknown as import("../../../src/ocr/llmloop/types.ts").DiffLike,
        template: {
          MaxTokens: 10000,
          MaxToolRequestTimes: 3,
