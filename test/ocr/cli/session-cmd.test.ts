...
import { JsonlWriter, SessionFilePath } from "../../../src/ocr/session/persist.js";
import {
  truncate,
  displayMode,
  describeFiles,
  ...
  withTempHome(() => {
    ...
    try {
      const sh = new SessionHistory(repoDir, "main", "test-model", {
        reviewMode: "commit",
        diffCommit: "abc123",
      } as unknown as import("../../../src/ocr/session/history.ts").SessionOptions);
      attachTestWriter(sh);
      sh.RecordReviewItemDone("a.go", "a.go", "a.go", "fp-a", [{ path: "a.go", content: "note" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment]);
      sh.Finalize();
...
test("runSessionComments text renders like review", () => {
  withTempHome(() => {
    ...
    try {
      ...
      attachTestWriter(sh);
      sh.RecordReviewItemDone("a.go", "a.go", "a.go", "fp-a", [
        { path: "a.go", content: "possible nil deref", startLine: 3, endLine: 5, severity: "high", category: "bug" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment,
      ]);
      sh.Finalize();
...
test("runSessionComments severity filter", () => {
  withTempHome(() => {
    ...
    try {
      ...
      attachTestWriter(sh);
      sh.RecordReviewItemDone("a.go", "a.go", "a.go", "fp-a", [
        { path: "a.go", content: "keep me", severity: "high" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment,
        { path: "a.go", content: "drop me", severity: "low" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment,
      ]);
      sh.Finalize();
...
test("runSessionComments JSON", () => {
  withTempHome(() => {
    ...
    try {
      ...
      attachTestWriter(sh);
      sh.RecordReviewItemDone("a.go", "a.go", "a.go", "fp-a", [
        { path: "a.go", content: "note", severity: "medium", category: "style" } as unknown as import("../../../src/ocr/model/review.ts").LlmComment,
      ]);
      sh.Finalize();
