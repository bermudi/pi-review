...
- OCR v1.9.8 commit `756203c` (carried through v1.9.9):
  root `--color` modes are resolved against the stdout TTY boundary, and text
  previews/comments/diffs use conditional ANSI styling while JSON/SARIF stay
  machine-clean.
- OCR v1.9.8 commit `66d71b2` (revalidated by v1.9.9):
  per-run injected progress routers reserve stdout for the final document and
  route human progress to stderr. Pi deliberately replaces OCR's process-global
  `stdout.Swap` with isolated composable router instances; the v1.9.9
  equivalence tests prove nested restoration and concurrent-run isolation.
- OCR v1.9.9 runtime boundary decisions: Pi owns settings/provider config and
  does not parse OCR app config. Pi 0.84.2's public `AgentSession` API has no
  provider-wire per-request `tool_choice`; the port instead verifies exact
  active-tool allowlists after every set and exposes filter tools only in their
  stage. This is not equivalent to OCR's required-tool wire field: the bounded
  filter fallback remains the intentional documented deviation.
- Production integrity follow-up: explicit `--model` uses Pi's public
  `ModelRuntime` against the configured agent directory and passes the resolved
  `Model` plus runtime to the session boundary. Review resume seals and
  validates identity before child persistence; reuse and background failures
  are propagated rather than silently treated as success.
- Model identity crosses the factory/resume seam as separate `provider` and
  `model` fields. The review factory explicitly maps sealed identity hash names
  to the resume contract before validation; it never compares collapsed
