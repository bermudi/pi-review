...
|---|---|---|
| `test/ocr-v193/llmloop/loop.test.ts` | `TestRunPerFile_TaskDoneExplicitDone`, `TaskDoneImmediately`, `UsesCompletionTokenLimit`, `TaskDoneFailed`, `InvalidTaskDoneStateRetries`, `TagsRequestsWithTaskSessionKey`, `TestRunner_RecordWarning`, `TestRunner_RecordUsage`, `MultiToolTurnIsOneRound`, `ContextCancelled`, `UnknownTool`, `TestExecuteToolCall_ArgumentsEdgeCases`, `EmptyToolCallsRetry`, `ThreeConsecutiveEmptyResults`, `MaxToolRequestsWithoutTaskDone`, `MaxRoundsGrace`, `CancelPreventsGrace`, `UncompressibleContextStopsWithCompression`, `GraceRoundToolDefs` in `internal/llmloop/loop_test.go` | implemented (19 pass) |
