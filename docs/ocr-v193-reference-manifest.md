...
| Gate 4 — scan/session/output | `verified` | `bun run verify:scan`; `verify:sessions`; `verify:outputs` | `20f0c459638d7b61b52f39e08eb7fa9bfe8f4c4f` | `verify:scan` 2 fixtures 11 assertions, pack `7786f51fc0ba71dfd2b38aa9c46730340f8c63d70e6f286859a23f689b932392`; `verify:sessions` 3 fixtures 29 assertions (checkpoint creation, kill+resume, mutation); `verify:outputs` 4 fixtures 20 assertions (text, json, json+agent, sarif); OCR tag object `4d796ae54cabdcf4e22b69ef502ed8871456a909`/commit `c35ddd7223f2b5540ce03aa43c9a25ef643fca27`; forbiddenImports=0; artifacts `/tmp/verify-scan-*`, `/tmp/verify-sessions-*`, `/tmp/verify-outputs-*` |
| Gate 5 — cutover | `verified` | `bun run verify:cutover` | `e2eb610037b425827e274856b836bb24677d92f1` | 8 fixtures 14 assertions, pack `91211b9aeeb38ff0cf7af029fb3f4fd6d28cec3e80d616e543e1151d64c15e60`; CLI default is parity (ocr-v193), library default `review`/`Reviewer`/`createReviewer` exports parity, legacy retained behind `--engine legacy` and `reviewLegacy`/`ReviewerLegacy`/`createReviewerLegacy`; default `pi-review review` reaches parity tool schemas, legacy constructor marker not emitted; `--engine legacy` emits legacy constructor marker; incomplete fixture exits nonzero; OCR tag object `4d796ae54cabdcf4e22b69ef502ed8871456a909`/commit `c35ddd7223f2b5540ce03aa43c9a25ef643fca27`; `notObservable`: `runtime provider traffic for the library default review() API`; forbiddenImports=0; artifacts `/tmp/verify-cutover-*` |
...
| `test/ocr-v193/llmloop/pool.test.ts` | `TestNewCommentWorkerPool`, `TestCommentWorkerPool_SubmitAndAwait`, `TestCommentWorkerPool_ErrorDoesNotBlock`, `TestCommentWorkerPool_Concurrency`, `TestCommentWorkerPool_AwaitEmpty`, `TestCommentWorkerPool_PanicIsIsolated`, `TestCommentWorkerPool_AwaitKeyWaitsForOwnKey`, `TestCommentWorkerPool_AwaitKeyConcurrentSubmitOtherKey`, `TestCommentWorkerPool_AwaitKeyUnknown` in `pool_test.go` | implemented (10 pass) |
...
| `test/ocr-v193/llmloop/loop-phase5.test.ts` | `TestExecuteToolCall_CodeCommentAsyncPool`, `TestExecuteToolCall_CodeCommentDiffResolved`, `TestExecuteToolCall_CodeCommentThinkingBackfill`, `TestRunPerFile_BackfillsThinkingFromReasoningContent`, `TestExecuteToolCall_TaskDone`, `TestExecuteToolCall_DynamicNotRegistered`, `TestExecuteToolCall_DynamicExecuteError`, `TestExecuteToolCall_DynamicSuccessRecordsResult`, `TestExecuteToolCall_DynamicParseError`, `TestCollectPendingComments_AwaitsPool`, `TestExecuteToolCall_CodeCommentOverridesHallucinatedPath` from pinned `loop_execute_test.go`, `loop_execute_more_test.go`, and `loop_test.go` | 11 pinned tests mapped; additional local relocation, ordering, deleted/context, suggestion, and concurrency regressions |
...
| `test/ocr-v193/diff/parser.test.ts` | No pinned parser tests mapped yet; omission is tracked in the parity matrix. | — |
