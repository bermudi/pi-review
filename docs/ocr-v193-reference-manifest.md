...
|| Gate 3 — core diff review | `building` | `bun run verify:core-review` | `1745079986ebba64d1adfae08ba5ed135b989fe5` | 3 fixtures, 29 assertions, pack `de1e6733`, artifacts `/tmp/verify-core-review-PItz0D`; positive `vertical-workspace-one-file-one-comment` and `vertical-mismatch-comment-content` (Family 1); positive `core-preview-selection-exclusion` (Family 4); remaining 6 families in progress |
