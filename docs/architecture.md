...
The Pi runner (`pi-runner.ts`) creates in-memory sessions with a minimal resource loader by default; when a session directory is configured it uses Pi's file-backed session manager instead, persisting each task's transcript as `.jsonl`. A persisted planning or main-review transcript can be resumed for a run narrowed to the same single selected file. The runner opens the transcript through Pi, checks its canonical repository path and persisted full task fingerprint (model, thinking, prompts, tool configuration, and budget), subscribes before adding a fresh prompt, and excludes historical messages from the new outcome. The resumed phase still needs a new successful atomic submission; later phases use fresh isolated sessions. Verification transcripts are deliberately not resumable because their exact-citation evidence ledger is host-only. It resolves the configured provider/model and checks authentication before creating a task session. It forwards only a narrow task outcome/event vocabulary, enforces abort and tool-start limits, sanitizes tool details, and disposes every session. Each task receives an explicit custom-tool allowlist; no default shell, edit, write, or unrelated Pi tool is exposed.
...
These choices favor precision and auditable failure states over maximum recall or feature parity with open-code-review's Go/provider/UI stack. Resume is phase-level conversation continuation only: it does not merge old run coverage, recover host-only tool state, or resume verification. The implementation has no full-repository scan mode, and no mechanical compile/test/formatter/linter phase. Workspace evidence can change during a long review because workspace reads are live after target acquisition; range and commit evidence are pinned. Target acquisition is bounded but whole-buffered as described above.
