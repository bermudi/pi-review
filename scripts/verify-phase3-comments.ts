...
function fail(msg: string, dir?: string): never { console.log(JSON.stringify({ phase:"phase3-comments", commit: currentCommit(), fixtures:[], assertions:0, notApplicable:[], privateImports:-1, result:"fail" as const, error:msg, artifactsDir: dir ?? null })); console.error(`[verify:phase3-comments] FAIL: ${msg}`); process.exit(1);}
async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({ phase:"phase3-comments", commit: currentCommit(), fixtures:[], assertions:0, notApplicable:[], privateImports:0, result:"fail" as const, error:"Phase 3 not yet implemented — awaiting Phase 2. See docs/ocr-v1.9.3-port-plan.md", artifactsDir })); console.error(`[verify:phase3-comments] BUILDING: awaiting Phase 2`); process.exit(1);
}
main().catch((e)=>{ const c=currentCommit(); const d=mkdtempSync(join(tmpdir(),"verify-phase3-")); console.log(JSON.stringify({phase:"phase3-comments",commit:c,fixtures:[],assertions:0,notApplicable:[],privateImports:-1,result:"fail" as const,error:e instanceof Error?e.message:String(e),artifactsDir:d})); process.exit(1);});
