...
async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({ phase:"phase5-scan-session", commit: currentCommit(), fixtures:[], assertions:0, notApplicable:[], privateImports:0, result:"fail" as const, error:"Phase 5 not yet implemented — awaiting Phase 2-4. See docs/ocr-v1.9.3-port-plan.md", artifactsDir })); console.error(`[verify:phase5-scan-session] BUILDING: awaiting earlier phases`); process.exit(1);
}
main().catch((e)=>{ const c=currentCommit(); const d=mkdtempSync(join(tmpdir(),"verify-phase5-")); console.log(JSON.stringify({phase:"phase5-scan-session",commit:c,fixtures:[],assertions:0,notApplicable:[],privateImports:-1,result:"fail" as const,error:e instanceof Error?e.message:String(e),artifactsDir:d})); process.exit(1);});
