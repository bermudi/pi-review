...
// src/ocr/llmloop/types.ts's minimal Template shape.
...
function resolveTemplateDir(explicit?: string): string {
  ...
  // When running from dist (bundled), the prompts live in src/ocr/template
  candidates.push(path.join(process.cwd(), "src/ocr/template"));
  // Also try relative to this file's cwd fallback
  candidates.push(path.resolve("src/ocr/template"));
  ...
  return candidates[0] ?? path.join(process.cwd(), "src/ocr/template");
}
